...
public class CaptureScreenShot implements ITestListener,IInvokedMethodListener,ISuiteListener{
		...
		printTestResults(result);
		 Object currentClass = result.getInstance();
	        ...
	        if (webDriver != null)
	        {
	        	...
	        	String filename = System.getProperty("user.dir")+"/SnowResource/screenshot/"+browserName+"_"+result.getMethod().getMethodName()+getCurrentTimeInstance()+".png";
			...
			this.nbExceptions++;
			Paragraph excep = new Paragraph(
					new Chunk("Selenium Exception Occurred", 
							new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE)).
							setLocalGoto("" + thr.hashCode()));
	...
	private void printTestResults(ITestResult result) {
		...
		if (result.getParameters().length != 0) {
			...
			//logback.info("Test Method had the following parameters : " + params, true);
	...
	logback.info("Test Status: " + status, true);
	...
	public void onFinish(ISuite suite){
		...
		if (totalTestCase == null) {
			...
			Paragraph p = new Paragraph("TEST EXECUTION SUMMARY", new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.BOLD));
			p.setAlignment(Element.ALIGN_CENTER);
			PdfPCell cell = new PdfPCell(p);
			cell.setColspan(5);
		...
		this.totalTestCase.addCell(cell);
			...
			Paragraph excep = new Paragraph(
					new Chunk(Integer.toString(totalFailed), 
							new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE)).
	...
	public void onStart(ISuite suite){
		try {
			PdfWriter.getInstance(this.document, new FileOutputStream(suite.getName()+".pdf"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		this.document.open();
		
		Paragraph p = new Paragraph(suite.getName() + " TESTNG RESULTS",
